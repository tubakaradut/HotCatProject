$(document).ready(function () {
    ...
    Codebase.helpers("jq-validation"), jQuery("#addCategoryForm").validate({
        submitHandler: function (form) {
            if ($("#addCategoryForm").valid()) {
                ...
                $.getJSON(window.location.origin + '/config.json').then(function (data) {
                    addCategory(categoryData, data.apiUrl);
                });
            }
        },
        rules: {
...
function getCategoryList(apiUrl) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var categoryParentOption = '<option value="null">-</option>'

            $.each(data, function (key, value) {
                var item = '<tr><td class="text-center">' + value.Id + '</td><td class="fw-semibold">' + value.CategoryName + '</td><td class="d-none d-sm-table-cell">' + value.Description + '</td><td class="d-none d-sm-table-cell">' + value.ParentCategoryName + '</td><td class="text-center"><button type="button" onclick="updateCategory(' + value.Id + ')" class="btn btn-sm btn-alt-warning" data-bs-toggle="tooltip" title="Düzenle"><i class="fa fa-pencil"></i></button><button type="button" onclick="questionModal(' + value.Id + ')" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="Sil"><i class="fa fa-trash"></i></button></td></tr>';

                categoryParentOption += '<option value="' + value.Id + '">' + value.CategoryName + '</option>';

                $("#categoryTableBody").append(item);
            });

            $("#categoryParentId").append(categoryParentOption);
...
//Kategori Ekleme
function addCategory(categoryData,apiUrl) {
    var token = localStorage.getItem("token");

    $.ajax({
        url: apiUrl + "Category/AddCategory",
        type: 'POST',
        headers: { "Authorization": "bearer " + token },
        data: categoryData,
        success: function (data) {
...
//Kategori Silme
function deleteCategory(id, apiUrl) {

    var token = localStorage.getItem("token");

    $.ajax({
        url: apiUrl + "Category/DeleteCategory/" + id,
        type: 'DELETE',
        headers: { "Authorization": "bearer " + token },
        success: function (data) {
